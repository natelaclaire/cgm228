...
1. Familiarize yourself with the tools and technologies for Java development:  
   * Setting up and configuring Java and the LibGDX library.  
...
1. Download and install BlueJ from [https://www.bluej.org/](https://www.bluej.org/).  
2. Test the installation by opening BlueJ and creating a simple "Hello, World\!" project.

#### **3\. Read Chapter 1 of the Textbook**

* Familiarize yourself with the concepts in Chapter 1 of [*Java Game Development with LibGDX*](https://drive.google.com/file/d/1qxHO-YE8ijOIClWuT0hHDs129mRW3JPd/view?usp=drive_link). This will guide you through setting up the Java development environment and running a basic LibGDX application.
...
1. Open the `1.1-hello-world` folder in BlueJ.  
2. Compile and run the `HelloWorld` program to verify it prints "Hello, World\!".  
3. Modify the program to display a custom message, such as "Hello, \[Your Name\]\!".  
4. Save, recompile, and rerun the program to verify your changes.

##### **7.2. HelloLauncher.java**

1. Open the `1.2-hello-world-image` folder in BlueJ.  
2. Run the `HelloLauncher` program to verify it displays the provided `world.png` image in a window.  
3. Modify the `HelloWorldImage.java` file:  
   * Change the referenced image file from `world.png` to another image (e.g., `custom_image.png`).  
   * Add your new image file to the project folder.  
...
1. Open the `1.3-interface` folder in BlueJ and run `ConcreteUse.java`.  
2. Review the code to understand how interfaces are used. Consult resources like [GeeksforGeeks](https://www.geeksforgeeks.org/interfaces-in-java/) for additional insights.  
3. Based on what you have learned, add comments to all three .java files in the `1.3-interface` folder explaining what is happening.  
4. Modify the `bookName()` method in the `JavaBook2.java` file to return the name of our textbook.  
5. Open the `1.4-abstract-class` folder in BlueJ and run `PayableInterfaceTest.java`.  
6. Explore how abstract classes are implemented alongside interfaces. Review [GeeksforGeeks](https://www.geeksforgeeks.org/abstract-classes-in-java/) for further reading.  
7. Create a new class named `HourlyEmployee` that extends `Employee`. You can copy `SalariedEmployee` and then modify it to have class attributes of `weeklyHours` and `hourlyPay` instead of the `weeklySalary`. Create the appropriate getters and setters for the new attributes and modify the constructor and `toString()` method appropriately. The `getPaymentAmount()` method should calculate the payment amount by multiplying the `hourlyPay` by the `weeklyHours`.

##### **7.4. Explore Static Methods and Classes**

1. Open the `1.5-static-class` folder in BlueJ and run `StringUtils.java`.  
2. Review examples of static methods and properties. Consult [CodeGym](https://codegym.cc/groups/posts/what-is-a-static-class-in-java) for more details on static members.  
3. Based on what you have learned, add comments to each of the static methods and attributes in `StringUtils.java` explaining what each is doing and the specific benefit(s) of declaring the method or attribute static.
...
| Criteria | Weight |
| ----- | ----- |
| Correct setup of Java and BlueJ | 20% |
...
2. **GitHub cloning errors**:  
   * Verify your internet connection and GitHub Desktop credentials.  
3. **LibGDX program errors**:  
   * Confirm that all required `.jar` files are included in the project.
